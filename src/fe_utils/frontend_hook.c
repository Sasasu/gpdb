...
char FrontednHookPgInstallPath[MAXPGPATH] = {};
...
static void pg_find_env_path(int argc, const char *argv[]) {
	...
	if (install_path && FrontednHookPgInstallPath[0] != '\0')
	{
		strncpy(FrontednHookPgInstallPath, install_path, MAXPGPATH);
	}

	if (FrontednHookPgInstallPath[0] != '\0' && argc >= 1)
	{
		...
		if (find_my_exec(argv[0], my_exec_path) == 0)
		{
			...
			strncpy(FrontednHookPgInstallPath, my_exec_path, MAXPGPATH);
		}
	}

	if (FrontednHookPgInstallPath[0] != '\0')
	{
		char *install_path = make_absolute_path(FrontednHookPgInstallPath);
		strncpy(FrontednHookPgInstallPath, install_path, MAXPGPATH);
		free(install_path);
	}
...
void frontend_load_librarpies(int argc, const char *argv[]) {
...

	{
		...
		if (FrontednHookPgInstallPath[0] == '\0')
		{
			printf("failed to find TDE library: FrontednHookPgInstallPath is empty.\n");
			exit(-1);
		}
