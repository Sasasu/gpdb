...
encryption_xlog_read_hook_type encryption_xlog_read_dir_hook = NULL;
	...
	while (LogwrtResult.Write < WriteRqst.Write)
	{
			...
			finishing_seg)
		{
			...
			if (encryption_xlog_write_hook)
			{
				...
				(*encryption_xlog_write_hook)(xlog_copy, npages, startoffset, openLogSegNo);
			}
...
retry:
	...
	if (encryption_xlog_read_dir_hook)
	{
		(*encryption_xlog_read_dir_hook)(readBuf, XLOG_BLCKSZ, targetPageOff, readSegNo);
	}
	if (r != XLOG_BLCKSZ)
	{
		char		fname[MAXFNAMELEN];
